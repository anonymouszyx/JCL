...
class MoCoUnlimitedKeysDefault(nn.Module):
    ...
    def forward(self, im_q, im_k, ratio = 1.0, idx_unshuffle = None):
        """
        Input:
        ...
        l_pos = torch.einsum('nc,nc->n', [q, k_means + 0.5 * torch.bmm(k_sigma, q.unsqueeze(dim=-1)).squeeze(dim=-1)]).unsqueeze(-1)
        # negative logits: NxK
        ...
        # sample_weights = torch.einsum('nc,nc->n', [q, k_means]).unsqueeze(dim=-1)
